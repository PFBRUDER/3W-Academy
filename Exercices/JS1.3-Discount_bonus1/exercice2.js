...
let vatRate = 20
let startJuly = 1593554400
let endJuly = 1596232799

function RemoveVatRateinJuly (orders, startJuly, endJuly, vatRate)  {
  for (let i=0 ; i<orders.length; i++) {
    let priceCommande = parseFloat(orders[i].price).toFixed(2);
    let dateCommande = parseFloat(orders[i].timestamp);
    if (dateCommande >= startJuly && dateCommande <= endJuly ) {
      // console.log(priceCommande)
      priceCommande = (priceCommande - priceCommande * vatRate / 100).toFixed(2)
      // console.log('==============')
      console.log(String(priceCommande))
    }
    else {      console.log(price = orders[i].price)
    }
  }
}

RemoveVatRateinJuly (orders, startJuly, endJuly, 33)
